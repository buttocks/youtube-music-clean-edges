// ==UserScript==
// @name         YouTube Music Clean Edges
// @namespace    ytmusic-clean-edges
// @version      3.5.0
// @description  Lightweight YouTube Music cleanup with compact layout controls, Audio Only mode, and automatic still-listening handling.
...
(() => {
  ...
  function createControls() {
    ...
    if (!cluster) {
      ...
      audio.addEventListener('click', event => {
        ...
        // Reset per-track guards when the user deliberately changes mode.
        lastAudioTrackKey = '';
        lastVideoRestoreKey = '';
  ...
  function trackKey() {
    ...
    // URL changes on most track changes; media src/title cover the cases where it doesn't.
  ...
  function start() {
    ...
    /*
     * Lightweight track watcher:
     * checks only a media element/title/URL once per second.
     * It never clicks Song repeatedly for the same track.
     */
    setInterval(() => {
      createControls();
      hookCurrentMedia();
